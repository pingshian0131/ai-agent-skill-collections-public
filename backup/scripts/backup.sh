#!/usr/bin/env bash
# OpenClaw backup script — creates timestamped tar.gz of ~/.openclaw
set -euo pipefail
...
case "${2:-backup}" in
  ...
  list)
    ...
    ls -lh "${BACKUP_DIR}"/openclaw-backup-*.tar.gz 2>/dev/null || echo "  (none)"
    ;;
  clean)
    KEEP="${3:-5}"
    TOTAL=$(ls -1 "${BACKUP_DIR}"/openclaw-backup-*.tar.gz 2>/dev/null | wc -l | tr -d ' ')
    ...
    ls -1t "${BACKUP_DIR}"/openclaw-backup-*.tar.gz | tail -n "${TO_DELETE}" | while read -r f; do
  ...
  upload)
    ...
    LATEST=$(ls -1t "${BACKUP_DIR}"/openclaw-backup-*.tar.gz 2>/dev/null | head -1)
    if [ -z "${LATEST}" ]; then
